...
class GeradorAnaliseCritica
{
    ...
    private function montarSecaoAnaliseDePrecos(): void
    {
        ...
        $introducao->addText($this->numeroTermoReferencia !== '' ? $this->numeroTermoReferencia : '[A PREENCHER]', ['bold' => true]);
        $introducao->addText('.');
    ...
    private function montarTabelaItem($secao, $lote, $item): void
    {
        ...
        $tabela->addCell($larguraTotal, array_merge($estiloCelulaTitulo, ['gridSpan' => 4]))
            ->addText('LOTE ' . $lote->numero . ' — ITEM ' . $item->numero, $fonteTitulo);
        ...
        $tabela->addCell($col4, $estiloCelulaCabecalho)
            ->addText('Cód. Sigadoc / Justificativa da Ausência', $fonteCabecalho, ['alignment' => Jc::CENTER]);
